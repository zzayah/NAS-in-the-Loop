#!/bin/bash
#SBATCH --job-name=f1_snas_sepang
...
RUN_ID="seed0_sepang_hpc_$(date +%Y%m%dT%H%M%S)"
RUN_DIR="data/safety-nas/compare-map-rerun-tp0/rerun-s-nas/$RUN_ID"

python safety-nas/control-logic.py --track SEPANG --output-dir "$RUN_DIR/nas" --session-id "$RUN_ID"
...
python safety-nas/compare-track.py --checkpoint-triple "${LEFT[0]}" "${TRACK[0]}" "${HEADING[0]}" --output-dir "$RUN_DIR/compare" --training-track SEPANG
