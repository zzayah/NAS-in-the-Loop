#!/bin/bash
#SBATCH --job-name=f1_snas_yas_marina
...
RUN_ID="seed0_yas_marina_hpc_$(date +%Y%m%dT%H%M%S)"
RUN_DIR="data/safety-nas/compare-map-rerun-tp0/rerun-s-nas/$RUN_ID"

python safety-nas/control-logic.py --track YAS_MARINA --output-dir "$RUN_DIR/nas" --session-id "$RUN_ID"
...
python safety-nas/compare-track.py --checkpoint-triple "${LEFT[0]}" "${TRACK[0]}" "${HEADING[0]}" --output-dir "$RUN_DIR/compare" --training-track YAS_MARINA
