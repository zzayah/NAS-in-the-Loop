#!/bin/bash
#SBATCH --job-name=f1_snas_hockenheim
...
RUN_ID="seed0_hockenheim_hpc_$(date +%Y%m%dT%H%M%S)"
...
python safety-nas/compare-track.py --checkpoint-triple "${LEFT[0]}" "${TRACK[0]}" "${HEADING[0]}" --output-dir "$RUN_DIR/compare" --training-track HOCKENHEIM
